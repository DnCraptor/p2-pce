...
static const char *get_drive_filename(int drive_idx) {
    if (drive_idx < 2) {
        return 0;//fdd_get_filename(drive_idx);      // FDD-0 / FDD-1
    } else {
        return 0;//ata_get_filename(drive_idx - 2);  // ATA0-0 .. ATA1-1
    }
}
...
static void draw_file_browser(void) {
    ...
    if (file_count == 0) {
        osd_print_center(FILE_Y + FILE_H / 2, "No disk images found in pce/", OSD_ATTR_DISABLED);
    }
