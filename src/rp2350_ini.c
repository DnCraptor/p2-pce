...
ini_sct_t *rp2350_load_ini(const char *path, const char* *err)
{
    ...
    if (res != FR_OK) {
        ...
        *err = "rp2350_ini: cannot open pce/config.ini";
        return NULL;
    }
    ...
    if (res != FR_OK || bytes_read != (UINT)size) {
        ...
        free(buf);
        *err = "rp2350_ini: read error pce/config.ini";
        return NULL;
    }
    buf[size] = '\0';
    ...
    if (!root) {
        free(buf);
        *err = "rp2350_ini: ini_sct_new returns NULL";
        return NULL;
    }
    ...
    if (ini_read_str(root, buf)) {
        ...
        ini_sct_del(root);
        free(buf);
        *err = "rp2350_ini: parse error pce/config.ini";
        return NULL;
    }
